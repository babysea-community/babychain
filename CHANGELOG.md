...
- Rebuilt the Chain Agent planner for BOLD per-step transformation (instruction version `2026-06-22.5`): each planned step now changes the scene/location, wardrobe, posture, lighting, color grade, and mood, keeping only the subject's face and body shape, instead of nudging the lighting on the same outfit and background. Image-to-video keeps the previous frame's scene and only adds motion and camera (and now opens already in motion, with no static hold of the input frame); video-modify boldly transforms the look (new background, wardrobe, and grade) while preserving the face, body motion, and camera move. The planner no longer writes the clip duration ("8-second", "over 8 seconds") into the prompt text.
...
- The running-step auto-cancel watchdog now follows BabySea's per-step SLA for both BYOK and BabySea mode: an image step is cancelled after 60s and a video step after 250s (replacing the previous fixed ~52-minute window), so stuck provider jobs are reclaimed far sooner.
