...
- The running-step auto-cancel watchdog uses BabySea's per-provider inference timeout for both BYOK and BabySea mode - image 60s, video 250s - because a BabyChain step hits a single provider per model. (BabySea's larger 210s/790s budgets are its 3-provider failover worst case, roughly 3x the per-provider timeout, which does not apply to one BabyChain inference.) When a step exceeds the limit it is canceled at the provider and marked `failed` with a clear timeout message ("The video generation timed out: the provider did not finish within the 250-second time limit, so it was canceled.") instead of the vague "did not reach a terminal state before the running deadline."
