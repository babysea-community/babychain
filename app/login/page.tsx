...
async function signInAction(formData: FormData) {
  ...
  const redirectTo = String(formData.get('redirect') ?? '/dashboard/canvas');
      ...
      <Card className="relative z-10 w-full max-w-md bg-card p-8 shadow-2xl ring-1 ring-border">
        <div className="mb-8 text-center">
          ...
          <p className="mt-3 text-sm leading-6 text-muted-foreground">
            BabyChain is a single-user workspace. Only the email configured in
            <code className="mx-1 bg-muted px-1 text-primary">OWNER_EMAIL</code>
            can sign in.
          </p>
          <p className="mt-3 text-sm leading-6 text-muted-foreground">
            Build your own BabyChain{' '}
            <a
