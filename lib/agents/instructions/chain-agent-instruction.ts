...
function chainAgentModelInstructions(options: { repairError?: string | null }) {
  return [
    ...
    '- DEFAULT TO VARIATION: even with no Creator Brief, the 3 suggestions MUST be 3 genuinely different professional directions, like 3 distinct shots from a real shoot. NEVER return three near-identical options that differ only by a small tweak. HOW you vary depends on the next step: when the next step GENERATES a new image (a base image or a restyling refine), vary the background/setting, lighting design, color grade, mood, wardrobe styling, hair/makeup, pose/posture, and lens/framing together. When the next step ANIMATES or EDITS an existing frame (image-to-video, or video modify), do NOT vary the scene or wardrobe at all - obey the IMAGE-TO-VIDEO rule below and vary only the motion, camera move, pacing/beat, and lighting and atmosphere evolution across the 3 options.',
    ...
    '- IMAGE-TO-VIDEO (and VIDEO MODIFY) OVERRIDES variation: the previous image is literally FRAME ONE of the clip and the video model continues from those exact pixels, so the clip can ONLY contain the subject, wardrobe, location, and background already in that frame. Keep the same person, face, clothing, and setting even when the background is blurred, dark, or minimal - a blurred or plain background stays that same blurred or plain background. NEVER invent a new location (no city street, alley, garden, forest, beach, studio, room, or any environment that is not already visible) and never change the outfit. The ONLY things you invent are MOTION and CAMERA: subject motion (turn, step, gesture, breathing, micro-expression), camera move (push-in, dolly, orbit, handheld drift, tilt), pacing, and how light, bokeh, hair, and fabric evolve over time. The 3 options are 3 different motion-and-camera takes of this one frame, not 3 different scenes.',
