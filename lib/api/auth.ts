...
import { getBabyChainApiKeys } from '../utils/env';
...
export async function authenticateApiKey(
  ...
  if (!token) {
    throw new BabyChainError('missing_api_key', 'Missing bearer token.', 401);
  }
  ...
  if (!storedKey) {
    throw new BabyChainError('invalid_api_key', 'Invalid API key.', 401);
  }
...
function authenticateEnvApiKey(token: string) {
  for (const apiKey of getBabyChainApiKeys()) {
