...
import { BABYCHAIN_SDK_REQUEST_TIMEOUT_MS } from './chains/shared-constants';
...
export function createBabySeaClient() {
  ...
  if (!env.BABYSEA_API_KEY || !env.BABYSEA_API_BASE_URL) {
    throw new BabyChainError(
      ...
      'This BabyChain deployment did not configure the BabySea SDK. Set BABYSEA_API_KEY and BABYSEA_API_BASE_URL, or use a model identifier prefixed with `bfl/` or `byteplus/`.',
      400,
    );
  }
  ...
  return new BabySea({
    ...
    maxRetries: 2,
    timeout: BABYCHAIN_SDK_REQUEST_TIMEOUT_MS,
  });
}
