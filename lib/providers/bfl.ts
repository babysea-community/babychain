 ...
 * Caveats
 * -------
 *   - `result.sample` URLs expire ~10 minutes after completion. BabyChain
 ...
 *     (`image_url`). BabyChain downloads upstream URLs (via the SSRF-safe
...
export function createBflProvider(config: BflProviderConfig): Provider {
  ...
  // Operator-supplied base URL overrides the regional host. BabyChain still enforces
