...
export function resolveServerByokConfig(): ByokRunConfig | null {
  ...
  if (env.BABYCHAIN_PROVIDER_MODE !== 'byok') {
    return null;
  }
  ...
  if (providers.length === 0) {
    throw new BabyChainError(
      'byok_not_configured',
      'BABYCHAIN_PROVIDER_MODE=byok requires BFL_API_KEY, ARK_API_KEY, DASHSCOPE_API_KEY, GEMINI_API_KEY or GOOGLE_API_KEY, OPENAI_API_KEY, or RUNWAYML_API_SECRET on the server.',
      500,
    );
  }
    ...
    if (!options.byokMode) {
      if (routed.babyseaCompatible === false) {
        throw new BabyChainError(
          ...
          `Model "${modelIdentifier}" is supported by BabyChain only in server-side BYOK mode. Set BABYCHAIN_PROVIDER_MODE=byok and configure the matching provider API key on the BabyChain server.`,
          400,
        );
      }
 ...
 * The public API request only authenticates to BabyChain itself.
 */
  ...
  switch (resolution.provider) {
    case 'babysea':
      if (byokMode) {
        throw new BabyChainError(
          'byok_credentials_missing',
          `Model "${resolution.modelIdentifier}" routes to the BabySea SDK provider, which is not BYOK-compatible with this server-side BYOK run. Use an Alibaba Cloud, BFL, BytePlus, Google, OpenAI, or Runway-backed model or set BABYCHAIN_PROVIDER_MODE=babysea.`,
          400,
        );
      }
    ...
    case 'alibabacloud': {
      if (!byokConfig || !byokConfig.providers.includes('alibabacloud')) {
        throw new BabyChainError(
          'byok_credentials_missing',
          `Model "${resolution.modelIdentifier}" requires DASHSCOPE_API_KEY on the BabyChain server.`,
          400,
        );
      }
      const env = getEnv();
    ...
    case 'bfl': {
      if (!byokConfig || !byokConfig.providers.includes('bfl')) {
        throw new BabyChainError(
          'byok_credentials_missing',
          `Model "${resolution.modelIdentifier}" requires BFL_API_KEY on the BabyChain server.`,
          400,
        );
      }
      const env = getEnv();
    ...
    case 'byteplus': {
      if (!byokConfig || !byokConfig.providers.includes('byteplus')) {
        throw new BabyChainError(
          'byok_credentials_missing',
          `Model "${resolution.modelIdentifier}" requires ARK_API_KEY on the BabyChain server.`,
          400,
        );
      }
      const env = getEnv();
    ...
    case 'google': {
      if (!byokConfig || !byokConfig.providers.includes('google')) {
        throw new BabyChainError(
          'byok_credentials_missing',
          `Model "${resolution.modelIdentifier}" requires GEMINI_API_KEY or GOOGLE_API_KEY on the BabyChain server.`,
          400,
        );
      }
      const env = getEnv();
    ...
    case 'openai': {
      if (!byokConfig || !byokConfig.providers.includes('openai')) {
        throw new BabyChainError(
          'byok_credentials_missing',
          `Model "${resolution.modelIdentifier}" requires OPENAI_API_KEY on the BabyChain server.`,
          400,
        );
      }
      const env = getEnv();
    ...
    case 'runway': {
      if (!byokConfig || !byokConfig.providers.includes('runway')) {
        throw new BabyChainError(
          'byok_credentials_missing',
          `Model "${resolution.modelIdentifier}" requires RUNWAYML_API_SECRET on the BabyChain server.`,
          400,
        );
      }
      const env = getEnv();
