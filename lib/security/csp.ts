...
/**
 * Builds BabyChain's static CSP from the active deployment environment.
 * BabyChain is API-first with a small public landing page, so a static policy
 * in next.config keeps the same allowlist discipline as Sherin without needing
 * middleware nonce plumbing. The CSP applies to HTML responses; JSON API
 * responses under /api/* receive Cache-Control headers via API_SECURITY_HEADERS.
 */
