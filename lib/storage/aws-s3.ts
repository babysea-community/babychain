...
import { createS3CompatibleStorageProvider } from './s3-compatible';
...
function bucketFromEndpointPath(pathname: string) {
  ...
  if (trimmed.includes('/')) {
    throw new Error(
      'AWS_S3_ENDPOINT_URL can include only the bucket path, for example https://s3.us-east-1.amazonaws.com/babychain.',
    );
  }
...
function required(value: string | undefined, name: string) {
  ...
  if (!trimmed) {
    throw new Error(
      `${name} is required when BABYCHAIN_STORAGE_PROVIDER=aws-s3.`,
    );
  }
