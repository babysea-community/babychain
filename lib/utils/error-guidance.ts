...
export function getErrorGuidance({
  ...
  if (code === 'byok_credentials_missing') {
    return guidance(
      'BabyChain cannot reach the selected provider with the current server configuration.',
      [
        'Set the provider API key required by the selected model, or switch BABYCHAIN_PROVIDER_MODE to babysea.',
  ...
  if (code === 'provider_unauthorized') {
    return guidance('The provider rejected the server-side credentials.', [
      'Verify the provider API key configured on the BabyChain server.',
  ...
  if (code === 'provider_unavailable') {
    return guidance('The provider is temporarily unavailable.', [
      ...
      'Check the selected provider status and BabyChain server logs if this repeats.',
    ]);
  }

  if (code === 'database_unreachable') {
    return guidance('BabyChain cannot reach its Aurora PostgreSQL database.', [
  ...
  if (code === 'provider_unexpected_response') {
    ...
    return guidance(
      'The provider returned a response BabyChain could not use.',
      [
