...
describe('jsonError', () => {
  it('does not capture expected 4xx errors', async () => {
    const response = await jsonError(
      new BabyChainError('unauthorized', 'Invalid token.', 401),
    );
  ...
  it('adds guidance for actionable request errors', async () => {
    const response = await jsonError(
      new BabyChainError(
        ...
        'Model "bfl/flux-1.1-pro" requires BFL_API_KEY on the BabyChain server.',
        400,
      ),
    );
  ...
  it('returns 5xx errors as API errors', async () => {
    const error = new BabyChainError('internal_error', 'Internal error.', 500);
  ...
  it('returns actionable guidance for database network failures', async () => {
    ...
    expect(body.error).toMatchObject({
      ...
      guidance: {
        summary: 'BabyChain cannot reach its Aurora PostgreSQL database.',
      },
    });
  });
